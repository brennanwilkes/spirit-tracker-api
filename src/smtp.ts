...
function buildMime(mail: Mail): { headers: string[]; body: string } {
  const subject = String(mail.subject || "").replace(/\r|\n/g, " ").trim();
  const text = String(mail.text || "");
  const html = typeof mail.html === "string" && mail.html.trim() ? String(mail.html) : "";
...
export async function sendMailSmtp(env: Env, mail: Mail): Promise<void> {
  ...
  try {
    ...
    const { headers: mimeHeaders, body } = buildMime(mail);
    const bodyStuffed = dotStuff(body);
    ...
    try {
      await withTimeout(client.cmd("QUIT"), timeoutMs, "SMTP QUIT");
    } catch {}
  } finally {
    await client.close();
  }
